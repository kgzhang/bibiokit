...
import type { DateRangeFilter } from '../core/query-manager';
import type { BaseFacet } from './base-facets';
...
class DateRangeFacet implements BaseFacet {
  ...
  getFilters(filters: DateRangeFilter[]) {
    const rangeFilter: { gte?: string; lte?: string } = {};

    if (!_.isUndefined(filters[0]?.dateMin)) {
      rangeFilter.gte = filters[0]?.dateMin;
    }

    if (!_.isUndefined(filters[0]?.dateMax)) {
