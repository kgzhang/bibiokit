import { GeoBoundingBoxFilter } from './../core/query-manager';
import { BaseFilter } from './base-filters';
...
class GeoBoundingBoxFilterClass implements BaseFilter {
  ...
  getFilters(filters: GeoBoundingBoxFilter[]) {
    return {
      geo_bounding_box: {
        [this.config.field]: {
  ...
  getSelectedFilter(filterSet) {
    const c = filterSet.getBoundingBox;
