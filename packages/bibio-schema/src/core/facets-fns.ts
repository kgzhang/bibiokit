import type { BaseFacet } from '../facets/base-facets';
import type { BaseFilter } from '../filters/base-filters';
import type QueryManager from './query-manager';
import type { SearchResponse } from './searchkit-request';
...
export const filterTransform = (
  ...
  return subFilters.length > 0 ? { bool: { must: subFilters } } : null;
};
...
export const getFacetsFromResponse = (
  ...
  const facetBucketKeys = Object.keys(response.aggregations).filter((aggKey) =>
    aggKey.includes('facet_bucket_'),
  );
