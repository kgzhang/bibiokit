...
import { SearchkitConfig } from './../resolvers/result-resolvers';
import { filterTransform } from './facets-fns';
...
export default class SearchkitRequest {
  ...
  constructor(
    ...
    this.dataloader = new dataloader(async (partialQueries) => {
      ...
      const esQuery = this.config.postProcessRequest
        ? this.config.postProcessRequest(query)
        : query;
