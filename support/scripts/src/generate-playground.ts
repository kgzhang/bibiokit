...
import { assertGet, camelCase, findMatches, invariant, pascalCase } from '@bibio/core-helpers';
import type { DtsCache } from '@bibio/playground';
...
const bibioPackageJson: PackageJson = loadJson.sync(
  path.join(packagesFolder, 'bibio', 'package.json'),
);
const bibioReactPackageJson: PackageJson = loadJson.sync(
  path.join(packagesFolder, 'bibio__react', 'package.json'),
);

const bibioPackageNames = Object.keys(bibioPackageJson.dependencies ?? {}).filter(
  (name) => name.startsWith('@bibio') && !name.startsWith('@bibio/pm'),
);
const bibioReactPackageNames = Object.keys(bibioReactPackageJson.dependencies ?? {}).filter(
  (name) => name.startsWith('@bibio') && !name.startsWith('@bibio/pm'),
);

interface PopulatebibioImports {
  /**
...
/**
 * Get the imports from the `bibio/` module and transform them into:
 *
 * - dts file mappings
 * - import keys
 * - the scoped name
 ...
 * files..
 */
async function populatebibioImports(
  props: PopulatebibioImports,
...
interface PackageDeclarations {
   ...
   * either scoped `@bibio/extension-bold` or a sub import
   * `bibio/extensions`.
   */
...
async function readDtsFile(filePath: string, key: string): Promise<DtsFileContents> {
  ...
  // to bibio and keep track of them. They can either be added manually via
...
async function preloadRequiredLibraries(dtsCache: DtsCache) {
  ...
  const preloadList: Array<[ModuleName, AbsolutePath]> = [
    ['@bibio/dev', baseDir('packages/bibio__dev/dist/declarations/src')],
...
interface ImportGroups {
  pm: bibioModuleMap;
  extensions: bibioModuleMap;
  presets: bibioModuleMap;
  core: bibioModuleMap;
  react: bibioModuleMap;
  unscoped: bibioModuleMap;
}

const externalModules = [
  ['@bibio/pm', '@bibio/pm'],
  ['@bibio/dev', '@bibio/dev'],
  ['bibio/extensions', 'bibio/extensions'],
  ['bibio/dom', 'bibio/dom'],
  ['react', 'react'],
  ['react/jsx-runtime', 'react/jsx-runtime'],
  ['react/jsx-dev-runtime', 'react/jsx-dev-runtime'],
  ['react-dom', 'react-dom'],
  ['prosemirror-dropcursor', '@bibio/pm/dropcursor'],
  ['prosemirror-gapcursor', '@bibio/pm/gapcursor'],
  ['prosemirror-history', '@bibio/pm/history'],
  ['prosemirror-inputrules', '@bibio/pm/inputrules'],
  ['prosemirror-keymap', '@bibio/pm/keymap'],
  ['prosemirror-model', '@bibio/pm/model'],
  ['prosemirror-schema-list', '@bibio/pm/schema-list'],
  ['prosemirror-state', '@bibio/pm/state'],
  ['prosemirror-tables', '@bibio/pm/tables'],
  ['prosemirror-transform', '@bibio/pm/transform'],
  ['prosemirror-view', '@bibio/pm/view'],

  ['@bibio/pm/dropcursor', '@bibio/pm/dropcursor'],
  ['@bibio/pm/gapcursor', '@bibio/pm/gapcursor'],
  ['@bibio/pm/history', '@bibio/pm/history'],
  ['@bibio/pm/inputrules', '@bibio/pm/inputrules'],
  ['@bibio/pm/keymap', '@bibio/pm/keymap'],
  ['@bibio/pm/model', '@bibio/pm/model'],
  ['@bibio/pm/schema-list', '@bibio/pm/schema-list'],
  ['@bibio/pm/state', '@bibio/pm/state'],
  ['@bibio/pm/tables', '@bibio/pm/tables'],
  ['@bibio/pm/transform', '@bibio/pm/transform'],
  ['@bibio/pm/view', '@bibio/pm/view'],
  ['@bibio/pm/suggest', '@bibio/pm/suggest'],
  ['@bibio/pm/paste-rules', '@bibio/pm/paste-rules'],
  ['@bibio/pm/trailing-node', '@bibio/pm/trailing-node'],
];
...
for (const [name, importName] of externalModules) {
  ...
  const computedImportName = importName.startsWith('bibio/')
...
async function generateCode(props: ImportGroups) {
  ...
  for (const meta of importGroups) {
    ...
    const isScoped = bibioPackageNames.includes(meta.name);
    const isReactScoped = bibioReactPackageNames.includes(meta.name);
...
import { PlaygroundImports } from 'bibio/playground';
import { ReactPlaygroundImports } from '@bibio/react/playground';
 ...
 * is to provide all the scoped bibio packages to the playground without
 * needing to import them all within the \`@bibio/playground\`.
 *
 * DO NOT EDIT: AUTO-GENERATED FILE
 * @see \`support/scripts/src/generate-playground.ts\`
 */

${bibioPackageNames.map((name) => `import * as ${camelCase(name)} from '${name}';`).join('\n')}

export const PlaygroundImports = {
  ${bibioPackageNames.map((name) => `'${name}': ${camelCase(name)},`).join('\n')}
} as const;
`;
...
${bibioReactPackageNames
  .map((name) => `import * as ${camelCase(name)} from '${name}';`)
  .join('\n')}

export const ReactPlaygroundImports = {
  ${bibioReactPackageNames.map((name) => `'${name}': ${camelCase(name)},`).join('\n')}
} as const;
...
`;
  ...
  return {
    ...
    exports,
    /** The playground exports in `bibio/playground` */
    playground,
    /** The playground exports in `@bibio/react/playground` */
    reactPlayground,
  };
}

// Where the generated file will be located.
const generatedFolder = baseDir('packages', 'bibio__playground', 'src', 'generated');
...
const playgroundPath = path.join(packagesFolder, 'bibio', 'src', 'playground.ts');
const reactPlaygroundPath = path.join(packagesFolder, 'bibio__react', 'src', 'playground.ts');
...
async function ensureActiveBuild() {
  const stat = await getFileStat(
    path.join(packagesFolder, 'bibio__core', 'dist', 'declarations', 'src', 'index.d.ts'),
  );
...
async function main() {
  ...
  const pm = await populatebibioImports({
    pattern: 'pm/package.json',
  });

  const extensions = await populatebibioImports({
    pattern: 'bibio__extension-*/package.json',
    excludedNames: ['@bibio/extension-react-native-bridge'],
  });

  const presets = await populatebibioImports({
    pattern: 'bibio__preset-*/package.json',
  });

  const core = await populatebibioImports({
    pattern: 'bibio__{core-*,core}/package.json',
  });

  const react = await populatebibioImports({
    excludedNames: ['@bibio/react-native'],
    pattern: 'bibio__{react-*,react}/package.json',
  });

  const unscoped = await populatebibioImports({
    excludedNames: ['jest-prosemirror', 'jest-bibio', 'a11y-status', 'test-keyboard'],
    pattern: '{a11y-status,create-context-state,multishift,prosemirror-*,bibio}/package.json',
  });
