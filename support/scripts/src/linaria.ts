 ...
 * place the css inside `@bibio/styles` package.
 */
...
import { camelCase, pascalCase } from '@bibio/core-helpers';
 ...
 * @param {string[]} paths
 */
const files = globby.sync(['packages/bibio__theme/src/*.{ts,tsx}'], {
  cwd: baseDir(),
  ignore: [
    '**/@bibio/styles',
    '**/website',
 ...
 * from.
 */
const outputDirectory = baseDir('packages/bibio__styles');
 ...
 * Where the package `@bibio/core` would have a scoped identifier of `core`.
 *
 * ```json
 * {
 *   "core": ["packages/bibio__core/src/styles.ts"],
 *   "react-social": ["packages/bibio__react-social/src/components/social-mentions.tsx"],
 * }
 * ```
 */
 ...
 * placed.
 *
 * @param {string} name - the scoped package identifier. `@bibio/core`
 * becomes `core`.
 */
...
export async function removeGeneratedFiles(): Promise<void> {
  ...
  // Delete css files 🤭
  await Promise.all([rm('packages/{@,}bibio/styles/*.css'), rm(tsFiles)]);
}

/**
 * Copy files from `@bibio/styles` to `bibio/styles`.
 */
export async function copyFilesTobibio(): Promise<void> {
  await cpy(['packages/bibio__styles/*.css'], 'packages/bibio/styles/');
}
