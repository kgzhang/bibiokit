...
import { deepMerge, isPlainObject, isString, object, omitUndefined } from '@bibio/core-helpers';
...
async function generateSizeLimitConfig() {
  ...
  for (const pkg of packages) {
    const limit = pkg['@bibio']?.sizeLimit;
    ...
    const ignore = [
      ...
      ...Object.keys(pkg.dependencies ?? {}).filter((name) => name.startsWith('@bibio/')),
    ];
...
function createAutoGeneratedFlag(folderName: string): object {
  return {
    __AUTO_GENERATED__: [
      ...
      `\`package.json > @bibio > tsconfigs > '${folderName}'\``,
      '',
...
async function resolveTsConfigMeta(
  ...
  const meta = pkg['@bibio']?.tsconfigs;
  ...
  if (src !== false) {
    ...
    configFiles.push({
      ...
      json: {
        ...
        ...other,
        references:
          pkg.name === '@bibio/support'
            ? [{ path: path.relative(path.dirname(filepath), dependencies['bibio'] ?? '') }]
            : references,
      },
    });
  }
  ...
  for (const [folder, config] of Object.entries(rest)) {
    ...
    if (folder.startsWith('__')) {
      ...
      const bibioPath = path.relative(
        path.dirname(filepath),
        path.join(dependencies['bibio'] ?? '', 'src'),
      );
      extraReferences.push({ path: testingPath }, { path: bibioPath });
    }
