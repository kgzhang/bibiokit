...
import {
  ...
  object,
  pascalCase,
} from '@bibio/core-helpers';
import type { IconTree } from '@bibio/icons';
...
// The icons which will are used within the `bibio` code base and included in
// the default bundle.
...
const coreIconsPath = baseDir('packages/bibio__icons/src/core-icons.ts');
const coreComponentsPath = baseDir('packages/bibio__react-components/src/icons/core.ts');
const allIconsPath = baseDir('packages/bibio__icons/src/all-icons.ts');
const allComponentsPath = baseDir('packages/bibio__react-components/src/icons/all.ts');
...
function generateComponentIconImports(iconNames: string[], type: IconCollection) {
  const importSource = 'core' === type ? '@bibio/icons' : '@bibio/icons/all';
  return `import {\n  ${iconNames.join(',\n  ')}\n} from '${importSource}';`;
}
 ...
 */
async function run() {
  log.debug(chalk`Generating icons for {grey \`@bibio/react-components\`}`);
